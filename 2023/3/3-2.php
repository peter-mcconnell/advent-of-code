...
function search_surround_for_symbol($int_val, $row_idx, $col_idx) {
  global $data;
  global $not_symbols;
  global $gears;
  $row_len = strlen($data[$row_idx]);
  for($r = -1; $r <= 1; $r++) { // search row -1, row, row + 1
    ...
    for($i = -1; $i <= 1; $i++) { // search col -1, col, col + 1
      ...
      $target = $data[$row_idx+$r][$c_col_idx];
      if(!in_array($target, $not_symbols)) {
        if($target == "*") {
          // handle gear logic
          $gear_key = $row_idx+$r . "_" . $c_col_idx;
          if(!array_key_exists($gear_key, $gears)) {
            $gears[$gear_key] = array();
          }
          array_push($gears[$gear_key], $int_val);
        }
        return true;
      }
    }
...
function int_if_valid($str_digit, $row_idx, $col_start_idx, $col_end_idx) {
  for($i = $col_start_idx; $i < $col_end_idx; $i++) {
    ...
    $val = intval($str_digit, 10);
    if(search_surround_for_symbol($val, $row_idx, $i)) {
      return $val;
    }
  }
